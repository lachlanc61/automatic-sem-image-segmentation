...
class WGAN:
    def __init__(self, root_dir, allow_memory_growth=True, use_gpus_no=(0, )):
        ...
        images = HelperFunctions.load_and_preprocess_images(input_dir_or_filelist=self.input_dir, threshold_value=0.5, normalization_range=(-1, 1), output_channels=1, contrast_optimization_range=None, dim_limit=True)
        for image in images:
            
            if image.shape[0] <= LIMIT_HEIGHT and image.shape[1] <= LIMIT_WIDTH:
    ...
    def start_training(self):
        ...
        # Callbacks
        cbk = GANMonitor(output_dir=os.path.join(self.output_dir, self.prefix), num_img=9, latent_dim=self.n_z)
    ...
    def simulate_masks(self,
        ...
        for i in tqdm(range(0, no_of_images)):
            ...
            random_latent_vectors = tf.random.normal(shape=(no_of_particles, self.n_z))
            samples = self.model(random_latent_vectors).numpy()
            ...
            img = np.zeros((img_height + d, img_width + d), dtype='uint8')
            noise_image = None

            if use_perlin_noise or use_random_rotation == 'PERLIN':
                opensimplex.random_seed()
                ix, iy = np.arange(0, perlin_noise_frequency, perlin_noise_frequency / img_width), np.arange(0, perlin_noise_frequency, perlin_noise_frequency / img_height)
            ...
            if use_perlin_noise:
                ...
                pos_values = (noise_image > (2 * perlin_noise_threshold - 1)) * 1.0  # remap noise_threshold to range [-1, 1] and use thresholded image (all positions > threshold are equally likely)
                ...
                pos_x, pos_y = np.random.randint(0, img_width, no_of_particles), np.random.randint(0, img_height, no_of_particles)
            ...
            if use_random_rotation == 'RANDOM':
                ...
                rotations = noise_image[pos_y, pos_x] * 360
            else:
            ...
            img = Image.fromarray(img[a:a+img_height, b:b+img_width] * 255)
