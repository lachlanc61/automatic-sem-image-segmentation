...
def patch_meantop(patch):
    ...
    FUTURE: should also be able to use this to estimate n. particles per image for training cyclegan
    """
    out=np.reshape(patch, -1)
...
def get_background_level(input_dir_bg):
    """
    loads background images and reports an average threshold value
    """
...
def padsplit(topad: int):
    """
    split an integer into two halves adding to original
        used to define amount to pad on each side
    """
    if (topad//2 == 0):
...
def load_and_preprocess_images(input_dir_or_filelist, threshold_value=None, normalization_range=(-1, 1), output_channels=1, contrast_optimization_range=None, dim_limit=False):
    ...
    #loop through once to get the maximum dimensions
    ...
    if dim_limit:
        ...
        if max_image_height > LIMIT_HEIGHT:
            max_image_height = LIMIT_HEIGHT
        if max_image_width > LIMIT_WIDTH:
            max_image_width = LIMIT_WIDTH
    ...
    for file in file_list:
        ...
        #wants an X,Y,1 matrix with explicit, flat 3rd dimension
        #pretty sure np.average is only 2D - suspect bug here if given RGB


        if not dim_limit or (image.shape[0] <= max_image_height and image.shape[1] <= max_image_width):

            #if any image is smaller than max, pad it out
            if image.shape[0] < max_image_height or image.shape[1] < max_image_width:
                ...
                image=np.pad(image, [(h1,h2),(w1,w2),(0,0)], mode='constant')
